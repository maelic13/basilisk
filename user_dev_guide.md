...
- [~] **6.5 SEE-quiet + capture futility — DONE 2026-07-01, SPLIT (commit `c3f1adc`); capture-futility SPRT owed.** Capture futility pruning **shipped ACTIVE** (canary-clean — KBNK/KQK have no captures): skip a capture when `eval + 200 + 200·lmr_depth + PieceValue[captured] + capHist/32 ≤ alpha`. Real behaviour change → bench **12,736,941 → 11,806,504**, startpos +33cp, 9/9 CTest. SEE-quiet pruning **exposed but INERT** (`QuietSeeDepth=0`): the naive base-table-`lmr_depth` port broke KBNK completely (no mate in 250 plies; every coeff 25–120 failed) — SF's `lmr_depth` here includes the history term that protects precise endgame quiets, which the base estimate lacks → deferred to 6.9 with the history-aware fix. **Maintainer SPRT owed:** `phase65-capfut` vs `phase64-deeper`, `elo1=3` @ 3+0.03 → keep if H1, else make inert + fold into 6.9. — Opus 4.8 high
- [ ] **6.6 Fail-low prior countermove bonus** (SF-sourced, simple classical form; weaker HCE evidence — SPRT decides). — Sonnet 5 medium
- [ ] **6.7 Fractional LMR (1024ths) + `lmr_tt_capture` input** (infrastructure for 6.9's finer SPSA; defaults converted exactly). — Fable 5 medium (alt: Opus 4.8 medium)
- [ ] **6.8 (optional) Qsearch quiet checks** (SF yes, Ethereal/Weiss no — mixed evidence; drop on first failure). — Sonnet 5 medium
- [ ] **6.9 wave2 joint SPSA** — the conserved compute, LAST: ~20–25 knobs (the 6.3 history-7 + **6.4's `PostLmrHistScale`/`DoubleExtMax` — both shipped inert, this is where their real values are found**, jointly with `LmrHistDiv`/`HistPruneCoeff` + LMP/NMP/ProbCut/qsearch margins/IIR/corr weights/razor depth/`lmr_cut_node_adj`). ~5000 iters @ 3+0.03, then **SPRT the converged result AND re-run the CTest suite** (6.2/6.3/6.4 all showed the KBNK/KQK canaries catch what a games-based SPRT alone might miss quickly). — Sonnet 5 medium
- [ ] **6.10 Phase-6 boundary validation**: cumulative SPRT vs `phase5final` + cross-TC non-reg `[-3,0]` @ 1+0.01 and 60+0.6. Passing → 1.8.0 release decision. — Sonnet 5 low
...
### Phase 7 - Non-NNUE ceiling: eval-refresh multi-cycle grind (PLAN.md §7.5; EXECUTE AFTER Phases 5–6, optional/evidence-driven — Sonnet 5 medium driving)
...
- [ ] **7.1 SF-distillation bootstrap — DO FIRST (the label re-center).** v17 was self-played *and* WDL-labeled by a ~300-Elo-weaker head. Re-center once against a far stronger teacher: the sibling **Hydra** (`D:/code/hydra`) already annotates the **quiet-filtered** beast set with **Stockfish @60k nodes → raw White-POV cp** (`annotate_sf.py` → `sf_train.csv`/`sf_holdout.csv`). Strong+dense+low-noise, and quiet-filtered so the deep score is a valid *static* target. **Integration:** convert `FEN;cp`→`FEN;wdl` via `1/(1+10^(-cp/400))` (parse_target rejects non-[0,1]); `--tune all` + `--tune-kingsafety`; bake; SPRT `elo1=3` @ 3+0.03 + gauntlet @ 10+0.1. **Watch eval SCALE** (search margins are cp-denominated — check startpos/material cp stay near current). May regress (off-policy + representability) → then skip to 7.2. **One bootstrap only; don't repeat SF.** — Fable 5 medium (alt: Opus 4.8 medium) pipeline; Sonnet 5 medium fit
- [ ] **7.2 On-policy self-play refresh cycle (the true-objective correction).** Must follow 7.1: self-play WDL optimizes Basilisk winning from positions *its own* search reaches, correcting distillation's off-policy + representability bias. Regen self-play with the strongest head (`datagen.ps1` → `extract_parallel.py`), **one joint low-lr refit** (`--tune all` + `--tune-kingsafety`), SPRT + gauntlet — NOT a re-stage. Fold in: phase-rebalance ~25/45/30 (v17 was 59% endgame) → 8–10M, quiet-filter, optional blended labels `λ·WDL+(1−λ)·σ(Basilisk-own score)`; ride-along re-fits: deferred **nonlinear KS re-fit** (now on correct data), sigmoid-K, tempo, lazy-margin, shelter/storm→danger fold. Expected ~+10–40 Elo. — Sonnet 5 medium
- [ ] **7.3 Iterate (further self-play cycles) + stop.** Repeat 7.2 with the now-stronger head; **stop** when a cycle yields `<~+8 Elo`, holdout loss stalls between regens, or the gauntlet doesn't move. Each accepted cycle is releasable (minor if real gauntlet Elo). Curve flattening = pure-HCE tuning spent → Phase 9 (NNUE). — Sonnet 5 medium
