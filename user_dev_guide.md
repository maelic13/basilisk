...
- [ ] **6.3 History formula v2: asymmetric linear bonus/malus** (SF + Weiss both; Basilisk's symmetric `min(d²,2048)` is the odd one out). 6 knobs seeded from Weiss (`bonus = min(251d−267, 2418)`, `malus = −min(532d−163, 693)`; gravity-range compatible) + `hist_ttmove_bonus` (~256). **Bundles the 6.1 mate guard (`75650d6`).** — Fable 5 medium (alt: Opus 4.8 medium)
...
- [ ] **7.0a Label & data-quality upgrade — DO FIRST (added 2026-07-01, SF/HCE audit finding).** The audit found **no missing eval feature** worth adding pre-NNUE (Phase 3/4 is feature-complete vs Ethereal/Weiss/SF-classical) — the HCE eval headroom is the **Texel loop's data**: (1) regen self-play with the Phase-6 head (v17's generator is now much weaker than the engine it trains); (2) **blended labels** `λ·WDL + (1−λ)·σ(search_score/K)`, λ≈0.5, grid on holdout — capture the per-position search score in `datagen.ps1`/extract (already computed, currently discarded; tuner accepts float targets since 4.0); (3) quiet-filter / qsearch-resolve sampled positions; (4) rebalance phases ~25/45/30 (v17 is 59% endgame) and scale to 8–10M; (5) per-cycle free riders: sigmoid-K refit, tempo, lazy-margin re-check, and the deferred **nonlinear KS re-fit** (now on correct data). — Fable 5 medium (alt: Opus 4.8 medium) for the pipeline; Sonnet 5 medium for fits
