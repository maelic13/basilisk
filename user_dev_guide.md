...
- [ ] **6.2 History: add 6-ply continuation table — CANDIDATE READY FOR SPRT 2026-07-01 (branch `phase6-6.2-history`).** Basilisk carried 1/2/4-ply continuation history; the sibling engine Rarog (same HCE lineage) also has a **6-ply** table — the real move-ordering signal Basilisk lacked (the bonus/malus *formula* was already equivalent, so this step is the table, not the formula). Added `cont_hist6_` mirroring `cont_hist4_` at all 8 sites (declare/alloc/clear/blend/`cont_hist_score` read/bonus+malus update/age); updated `bonus/3` (best) & `malus/3` (other quiets), read at `/3`. The 4 sentinels only cover `(ss-4)`, so all three `(ss-6)` accesses are guarded by `ss - ss_arr_ >= 6` (no stack-layout change — `MAX_QSEARCH_PLY=10` already makes the `+8` headroom tight). **Bundles the 6.1 mate-score guard** (`75650d6`). Candidate `basilisk-phase62-history-pext-pgo.exe` (v1.8.0). Behaviour-changing: **bench 13,503,085 → 11,266,326** (~−16% nodes, EBF 2.854 → 2.814 — better ordering + larger history magnitude into LMR/history-pruning), startpos depth-14 sane (+63cp, `g1f3`). **NEXT: maintainer SPRT** `elo1=3` at `3+0.03`, `phase62-history` vs `phase61-ttbound` (the 6.1/development head). If H1 → merge to `development`. — Sonnet 4.6 medium
- [ ] 6.2b (optional) history bonus/malus *formula* asymmetry / SPSA — deferred to the Phase-6 wave2 SPSA (the formula itself is already Rarog-equivalent; expose+tune only if wave2 has headroom).
