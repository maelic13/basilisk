...
expect ~half the fast-TC gain to survive at LTC.** **▶ NEXT (post-1.8.0), per the
closed Step 7.0 ceiling analysis:** residual non-NNUE headroom ≈ **+25–35 LTC**
(self-play cycles ~+7 LTC each while they pay ≥+8 fast — optionally bank a 1.9.0);
below that, **Phase 9 NNUE (+200–400) is the only lever left**. No Phase 8, no
king-bucketed PSTs. The bullets that follow are the Phase-1 historical
record.
...
> | 7.4 histshape SPSA ⏭️ | **SKIPPED, unmeasured** (est. 0…+12/~+4 fast ≈ ~+2 LTC) | 12–18h declined on cost/benefit | ~50% + SPSA-at-maturity 0-for-2 | reopen only if cycling stops, no NNUE imminent |
...
- [x] **7.0 Non-NNUE ceiling analysis — ✅ CLOSED 2026-07-08 (post-1.8.0), priors replaced by measurement.** The self-play ratchet massively out-delivered (~+81 fast-TC over 5 cycles vs ~+30 prior; shallow ~10%/cycle taper) but **~55% compresses at LTC** (+93 fast → ~+40 LTC, colosseum gauntlet). **Residual non-NNUE headroom from the 1.8.0 head ≈ +25–35 LTC** (cycles 6–10 ≈ +52 fast ≈ +25 LTC before the <+8 stop rule; skipped 7.4 SPSA ≈ +2 @50% wash). The shelter/storm→danger fold is **measured-low-value, closed** (`ks_shelter_storm` was live in all five KS re-fits; the tuner put exactly 1 unit on it while keeping the parallel linear terms). Placement ≈ Rarog/Shredder-12; cashing the residual reaches ≈ HIARCS/SF-2900 territory — the gap to SF11-class 3440 is fishtest-scale maturity, unreachable with these levers. **Stop rule: cycle while ≥+8 fast (optionally bank a 1.9.0); below that — or whenever appetite exists — go straight to Phase 9 NNUE (+200–400). No king-bucketed PSTs (NNUE-gateway trap), no Phase 8.** NNUE activation energy is now low: the datagen/quiet-filter/SF-annotation/extract pipeline is built and battle-tested — exactly what a small net needs. — analysis step, no games
...
- [x] **7.3 Iterate (further self-play cycles) + stop — ✅ CLOSED for 1.8.0 at three cycles, all ACCEPTED:** cycle 1 **+19.51 ± 7.04** (merged `9741070`), cycle 2 **+18.29 ± 6.76** (merged `58432c5`), cycle 3 **+15.32 ± 6.09** (merged `eadbc3e`, bench 12,661,251 = the 1.8.0 head) — H1/LOS 100% each, tapering ~10%/cycle but never near the `<+8` stop rule; iteration was paused for the 7.5 LTC gauntlet (user decision: "cycle 5, then LTC gauntlet") rather than stopped by the rule. **⚠️ The stop signal is the SPRT, NOT the holdout-MSE stall** — the linear eval barely moved each cycle (movers ±1/±2) yet gained +15–20 (the KS funnel keeps re-shaping: safety_table tail 342→351→373→398→407). **Post-1.8.0 the same loop continues as the 1.9.0 track** (cycle 6 = branch `phase73d-selfplay`, SPRT pending), governed by the closed Step 7.0 stop rule: cycle while ≥+8 fast; below that → Phase 9 NNUE. — Sonnet 5 medium
- [x] **7.4 histshape SPSA — ⏭️ SKIPPED 2026-07-08 (unmeasured — NOT "rejected"), and structurally unrunnable while cycling.** Never run, so it "brings no Elo" only in the sense that we declined to spend 12–18h + a confirming SPRT on an estimated **~+4 fast ≈ ~+2 LTC at ~50% wash** (family SPSA-at-maturity base rate 0-for-2: the 5.8 TM wash + Rarog's 30h negative). *Skipped ≠ rejected:* a rejection (like 6.2's −7.70) is a measured verdict that closes a mechanism forever; this is an unmeasured pass on cost/benefit grounds. **It also cannot legitimately run right now:** its own premise (§0.5 — search constants are cp-denominated, tune once at the FINAL eval scale) is violated while self-play cycles keep re-fitting the eval (the 1.9.0 track is active), and if the path after cycling is Phase 9 NNUE, the eval scale changes wholesale and any histshape tune would need redoing anyway. **Reopen only if all three hold:** (a) HCE cycling has permanently stopped, (b) NNUE is not imminent, (c) a last ~+2 LTC is worth ~a day of compute. The infrastructure stays ready (`config_histshape.json`, 11 always-live dims from current defaults; recipe preserved in PLAN §7.5 Step 7.4); the gated 6.4/6.5/6.8 mechanisms stay inert regardless. — Sonnet 5 medium
