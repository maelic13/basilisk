...
- [~] **6.9 wave2 joint SPSA — INFRASTRUCTURE READY 2026-07-02 (commit `74bf5f0`); maintainer SPSA run pending.** `tools/spsa_configs/config_wave2.json` = **24 knobs**: the 6.3 history-7 + 6.4's `PostLmrHistScale`/`DoubleExtMax` (both shipped inert, this is where their real values are found) + 6.5's `CapFutDepth/Base/Coeff`/`QuietSeeDepth/Coeff` + `QsearchCheckCap` (6.8) + `LmrTtCapture` + `LmrCutNodeAdj` (6.7, live headroom) + `LmrHistDiv`/`HistPruneCoeff` (coupled to history magnitude per 6.2/6.3). **Excludes** the 12 already-accepted legacy pruning constants and the never-wired legacy wishlist (LMP/NMP/ProbCut/qsearch-margins/IIR/corr-history — a possible wave3, not silently bundled here); see `spsa_configs/README.md` for the full table + rationale. Baseline binary `basilisk-phase69-wave2base-pext-pgo.exe` built (Phase-6 head, bench 12,736,941, all 24 knobs verified advertised); `setup_spsa.ps1 -ConfigGroup wave2 -EngineSuffix phase69-wave2base` run end-to-end and verified. **NEXT: maintainer runs** `cd tools/weather-factory && python main.py` (~5000 iters @ 3+0.03), then **SPRT the converged result vs `phase69-wave2base` AND re-run `ctest`** before accepting (6.2/6.3/6.4/6.5/6.8 all showed the canaries catch what SPRT alone might miss quickly). — Sonnet 5 medium
