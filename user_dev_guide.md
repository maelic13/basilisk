...
- [ ] **6.1 TT-bound eval refinement — CANDIDATE READY FOR SPRT 2026-07-01 (branch `phase6-6.1-ttbound`, rebased onto the Phase-5 head).** In `negamax`, a separate `eval` for *pruning decisions only* (RFP/razoring/NMP/futility) prefers the TT score when its bound proves it tighter than the corrected static eval (exact / fail-high above / fail-low below); `ss->eval`/`static_eval` stay raw so `improving` + correction-history are unaffected. Mirrored in qsearch stand-pat. **Rebased cleanly onto `development` (16bd485)** — kept OFF `development` until SPRT-validated. Candidate binary `basilisk-phase61-ttbound-pext-pgo.exe` built (v1.8.0, bench-only PGO). Behaviour-changing on the 40-position bench: **fingerprint 13,466,664 → 13,503,085** (+0.27% net, but the pruning cuts the hardest position hard — top-pos share 19.6% → 7.2%, position 39 dropped 2.64M → 0.97M nodes, exactly the EBF-gap-closing intent). startpos depth-14 sane (+33cp, `b1c3`). CTest expected 9/9 (rebase clean, change is pruning-only — perft/endgame paths untouched). **NEXT: maintainer SPRT** `elo1=3` at `3+0.03`, `phase61-ttbound` vs `phase5final` (the Phase-5/development head). If H1 → merge to `development`. — Sonnet 4.6 medium
