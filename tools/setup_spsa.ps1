...
.DESCRIPTION
    ...
    every 10 iterations to tuner\state.json so you can resume at any time.
...
.PARAMETER Iterations
    ...
    Default: 5000.  The tuner uses tc=3+0.03, matching sprt.ps1, so runs are
    slower than the old tc=1 setup but avoid the tune->confirm TC mismatch.
