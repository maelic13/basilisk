...
.DESCRIPTION
    ...
    Conditions (chosen to match the Little Blitzer deployment exactly):
      - st=0.1  -> fixed 100 ms/move.  Fixed movetime, so engine time-management
                   is NOT exercised (correct: it is not used in deployment either).
    ...
    IMPORTANT — concurrency and fixed movetime:
      ...
      to the 30 logical processors — that adds noise for fixed movetime.
