...
|---|---|---|---|---|
| BAS-S01 | TT-bound-aware pruning evaluation used a proving TT bound for RFP, razoring, NMP, futility and qsearch stand pat while preserving raw/corrected eval for improving and correction updates. | **Accepted, +7.18 Elo** in the 1.8.0 search state. | In that state, separating pruning evidence from training evidence improved their cooperation. Preserve this separation when redesigning result evidence in Plan 5.2–5.4. | `CHANGELOG.md` 1.8.0 |
...
| BAS-S08 | Blanket removal of the check extension against the `hcefinal`-tuned head. | **Rejected, −10.17 ± 6.52 over 4,682 games; reverted.** | The result may reflect consumers tuned around the extension, not a universal need for it. Retry only as the jointly refitted Phase-5.4/5.5 bundle; Rarog's opposite result is a prior, not a verdict. | `CHANGELOG.md` 1.9.1 |
| BAS-S09 | Corrected an LMR reduction gate that consumed `gives_check` after making the move. | **Standalone rejected, about −20 Elo; reverted/deferred.** | The incorrect path may have acted as an aggressive-reduction heuristic and the surrounding constants were fitted with it. Reintroduce only inside unified pre-move evidence plus joint refit in Plan 5.5/5.10. | `CHANGELOG.md` 1.9.1; `analysis/search_analysis.md` |
...
| BAS-S11 | Cutoff-count LMR was considered after Rarog tested a full LMR-family retune. | **Not implemented standalone; imported Rarog result was −7.78 ± 8.00.** | `cutoffCnt` is not a mandatory improvement. It may be tested only as a diagnosed coordinate within Plan 5.5/5.10, followed by ablation. | legacy plan at `8dc0a24^`; Rarog `CHANGELOG.md` |
| BAS-S12 | Cuckoo repetition and post-LMR history candidates were tested on the old unpinned harness. | **Rejected/closed for the old state; measurements carried harness uncertainty.** | The old conditions leave residual uncertainty, but repeated retries have low expected value. Reopen only if Phase 5 diagnostics identify the exact missing consumer and pre-register one terminal test. | legacy plan at `8dc0a24^` |
...
| BAS-R02 | SPSA of nine time-management constants on the old root model. | **Neutral, +0.88 ± 4.03 over 12,262 games; reverted.** | The tested constants appeared near a local ceiling in that root model. A retry is justified only after Plan 5.7 changes the confidence inputs. | `CHANGELOG.md` 1.8.0 |
...
| BAS-P07 | `origin/arm_fix` aligned the TT allocation to 64-byte cache lines. | **Unverified experiment:** no AArch64 build, bench, sanitizer or NPS evidence. | Plausible layout does not establish ARM correctness or speed. Plan 5.8 inventories the patch and tests isolated allocation/alignment variants per target. | `PLAN.md` 5.8; branch `origin/arm_fix` |
...
|---|---|---|---|
| BAS-X01 | Check-extension removal was +30.75 Elo in Rarog but −10.17 ± 6.52 in Basilisk. | Search mechanisms can be jointly de-tuned; copy the experiment design, not the verdict. | 5.4, 5.5, 5.10 |
...
| BAS-X03 | Rarog's full `cutoffCnt`/LMR-family candidate lost −7.78 ± 8.00 despite its tuning trajectory. | Tuner success can select a self-play-local optimum. Treat `cutoffCnt` as an optional diagnosed coordinate, not required parity. | 5.5, 5.10 |
| BAS-X04 | Rarog gained +22.13 from history bonus/malus work and +6.01 from a broader history bundle. | Result-source attribution and consumer normalization may unlock history value, but Basilisk's 6-ply channel already showed duplication risk. | 5.2, 5.6 |
| BAS-X05 | Rarog's accepted SMP rework was +102.78 ± 16.38 at 4T, much larger than Basilisk's Phase-9 result. | The gap suggests different baseline defects, not a transferable Elo budget. Compare root ownership, helper limits, history merge and TT interaction diagnostically. | 5.7, 5.9, 9.0 |
| BAS-X06 | Rarog's bench-identical speed wave gained +10.35% NPS and +20.31 ± 7.13 Elo at STC. | It corroborates that wall-clock speed can convert to strength near this host/TC, while leaving LTC and ISA transfer unknown. | 5.8, 5.9 |
| BAS-X07 | Rarog `arm_fix` adds an AArch64 prefetch path and a runtime-hoist idea; x64 evidence was bundled and ARM untested. | Isolate the prefetch intrinsic/fallback and initialization hoists; do not copy the Rust implementation or claim ARM speed. | 5.8 |
| BAS-X08 | Rarog's parity audit emphasizes shared `MoveEvidence`, prospective depth and correction attribution. | These abstractions may reduce contradictory consumers in Basilisk if the telemetry confirms the same failure modes. | 5.1–5.6 |
...
|---|---|---|
| BAS-S08, BAS-S09, BAS-S11 | Unified pre-move evidence and prospective-depth model implemented; consumers included in the single joint fit; post-fit ablations registered. | 5.4, 5.5, 5.10 |
| BAS-S07, BAS-S10, BAS-S12 | Diagnostics show a distinct source/consumer gap that the existing history tables cannot represent. | 5.1, 5.2, 5.6 |
| BAS-R02, BAS-R03 | Root-confidence inputs or NNUE score scale materially change. | 5.7, 7.6, 8.3 |
| BAS-P04, BAS-P05, BAS-P06 | A new profile demonstrates changed reuse, cache pressure or PGO coverage. | 5.9 or 9.1 |
| BAS-P07, BAS-X07 | Cross-build runner and physical non-x64 machines are available; isolate one variant per A/B. | 5.8 |
