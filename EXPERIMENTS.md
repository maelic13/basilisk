...
| BAS-E08 | Frozen 770-position Syzygy truth baseline: accepted head `294a3e2` versus Stockfish `dev-20260716-ebcea3ef`, both at 60k nodes, 1T, 16 MB, engine TB disabled and no score adjudication. | **Baseline observation:** Basilisk converted 293/480 clean wins (61.04%) versus 389/480 (81.04%); paired matrix 277 both, 16 Basilisk-only, 112 reference-only, 75 neither. | Finite-node conversion ceilings must be empirical, not 100%. Largest measured deficits were KBP-K, KQ-KR, KNN-KP, KBN-K and KQ-KRP. Move-level rates are descriptive until the paired confidence contract is set. | `tools/results/endgame-truth-6.0.b/{basilisk,reference}.json`; PLAN 6.0.b |
