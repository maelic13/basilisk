...
| BAS-P09 | Windows x64 PEXT+PGO compiler comparison at `5b92458`, dirty build-support patch `33b60cc9`: MSVC 19.40.33816 (`/O2 /GL /LTCG /favor:INTEL64 /arch:AVX2`, `/GENPROFILE` -> depth-13 bench -> `/USEPROFILE`) versus MSYS2 Clang 22.1.8 (`-O3 -march=native -mbmi2`, LTO+PGO). Two independent builds/arm, Hash 64, 1T, logical CPU 18 (Windows efficiency class 1), High priority, 8 alternating depth-13 rounds. MSVC SHA-256 prefixes `C7DA3551`/`D8174BC6`; Clang `C557183B`/`1F528F4D`. Self-pair first passed at +0.17%, CI [-1.46%, +2.25%]. | **MSVC rejected as the Windows production default; build support retained.** Exact fingerprint 12,568,898 on both arms. Pooled medians: MSVC 2,605,492 NPS, Clang 2,952,260 NPS; MSVC -11.75%, bootstrap 95% CI [-15.54%, -9.23%], 0/8 paired round wins. WAC parser/SAN/floor test passed 5/5 after splitting the embedded corpus for MSVC's literal limit. No games were run; this is a throughput/toolchain verdict, not Elo evidence. | On this Core Ultra 7 165H P-core and engine revision, MSVC's PGO codegen is materially slower despite equivalent work. Keep Clang for Windows releases; retry MSVC only after a materially newer toolset/codegen change, and repeat on other CPU families before generalizing. | `msvc-release-pext` preset; this commit |
| BAS-P10 | GCC PGO advertised `-fprofile-generate/-use`, but the orchestrator always searched for Clang `.profraw` files and invoked `llvm-profdata`; a separate final build tree would also change GCC's object-path-based profile names. | **Tooling repair accepted; search unchanged.** The orchestrator now branches on the configured compiler ID, verifies GCC emitted `.gcda`, and reconfigures/rebuilds the same object tree under `-fprofile-use`. MSYS2 GCC 16.2.0 produced and consumed 15 `.gcda` files without missing-profile diagnostics; the final native PGO binary preserved the exact 12,568,898-node bench. Clang's independent `.profraw`/`.profdata` route remains separate. | PGO formats and profile lookup identities are compiler-specific. A flag being accepted is not proof that training output was consumed; retain a real GCC PGO build in routine coverage. | `cmake/pgo-build.cmake`; `CMakeLists.txt` |
| BAS-P11 | Main CI ran only after pushes to `master` or manual dispatch, while AVX2, PEXT, Linux ARM64 and Windows ARM64 were exercised only by the release workflow. Agent changes on the active development/PR path could therefore bypass the strongest available deterministic gates. | **Delivery repair accepted; search unchanged.** CI now triggers for `development`, pull requests, `master` and manual runs. Its release matrix runs full CTest, TUNE compilation and a cross-OS/architecture/ISA bench fingerprint for portable, AVX2 and PEXT x86-64 plus Linux, Windows and macOS ARM64. A separate Linux GCC job generates, consumes and searches with a real PGO profile. Local Windows Clang route checks built all three x86 tiers with TUNE enabled and returned the same depth-10 fingerprint (3,022,019 nodes); target-native ARM and CI-service semantics remain verified by the workflow itself. | Release-only coverage detects defects after integration. Keep the development matrix aligned with every published platform/ISA tier, and exercise profile generation rather than only accepting PGO flags. | `.github/workflows/ci.yml` |
...
| BAS-C05 | A TT slot published its 64-bit payload separately from a plain 16-bit key, so an SMP reader could accept an old key with a new score/bound payload. | **CLOSED 2026-09-07: the defect is real, BOTH repairs were rejected on measured cost, and the incoherence is now an ACCEPTED, RECORDED risk.** The exposed consumers are `src/search.cpp:1589`, which returns `tt_score` as the node value on a depth-and-bound match with no verification, and `src/search.cpp:1967`, which multicuts on a singular beta derived from it; worst case is a foreign mate score reaching the root. The pre-repair comment claiming a mismatched pair was "harmless ... bounds validated" was false; `src/tt.h` and `DESIGN.md` s3 now record the risk instead. Repair 1, `key16 ^ fold16(payload)`: The tag bound the pair correctly -- torn-pair tests passed, the old scheme failed their negative control, 25 collision-stress repetitions covered 10M contested writes, ASan/UBSan suites passed and the 1T bench stayed exactly 12,568,898 -- but it cost **-3.91% NPS at 1T with identical node counts** (6 strictly alternating `bench 13` repetitions per arm on the frozen gate binaries; means 3,666,005 vs 3,522,690; the two arms' ranges do not overlap). At BAS-P01's ~2 Elo per 1% NPS that predicts ~-7.8 Elo, and the registered 4T SPRT read `-11.30 +/- 9.64` (nElo `-17.87 +/- 15.23`, LLR -1.27, 2,020 games, zero forfeits, maintainer-stopped). Cause: `probe_copy` can no longer reject a slot on a 2-byte compare -- it must load the 8-byte payload and fold it for every scanned slot, with the fold on the branch's dependency chain. **The 4T reading is separately unusable as a verdict**: BAS-M02 and `tools/sprt.ps1` require fixed-N 10k at 4T and a per-thread-count `-Mode calibrate` null, `-use-affinity` is dropped at Threads>1, and no 4T null exists in `tools/results/`; the 1T NPS isolate is what condemns the implementation. H0 does not license restoring incoherent reads. Repair 2, IMPLEMENTED then REVERTED, `2bf43fb` (`analysis/tt_publication_coherence_v1.md`): `key16|score16|eval16|depth8|flag_age8` packed into the single atomic 64-bit word (exactly 64 bits, none spare) with `move16` moved to the separate 16-bit field, making the tear unrepresentable for every bound-deciding field. Bench exactly 12,568,898 on both arms; release and ASan/UBSan CTest 12/12. **Its throughput prediction was FALSIFIED.** Frozen prediction was pooled NPS in `[-0.5%, +1.5%]`, central `+0.3%`; measured **-1.22%, 95% CI [-1.72%, -0.76%]**, A faster in 1/16 rounds, per-build medians agreeing to 0.03%/0.14% within each arm (`tools/nps_ab.ps1`, 2 pooled PGO builds per arm, 16 alternating rounds, identical fingerprints). At BAS-P01's ratio that is ~-2.4 Elo. The frozen packet named this exact failure mode and range in advance -- the baseline's three 2-byte key loads reject more cheaply than three 8-byte word loads -- so the mechanism was predicted even though the headline was not. Caveat: the instrument's self-pair read `+0.33%` against a `0.30%` tolerance (CI `[-0.27%, +1.05%]`), marginally failing, which is small against the 2.7 percentage points separating the two candidate encodings but not against the acceptance band. **Maintainer decision: revert to plain-key and accept the risk.** 64 bits cannot hold key+score+eval+depth+flag+move (80 bits needed), so coherence is not purchasable at 10-byte density; ~2.4 Elo is a quarter of the entire BAS-P01 speed wave, the defect has never cost a measured game, and Stockfish ships this same tolerance in this same structure. No game gate was run: a `[-5,0]` SPRT on a true ~-2.4 sits between the hypotheses and cannot terminate. | Search must validate a shared record as one logical publication before consuming its bound -- the exposed consumers are the direct TT cutoff at `src/search.cpp:1589` and the singular multicut at `src/search.cpp:1967`, both of which return a TT-derived score. Exact 1T identity cannot exercise the race, but it does not license skipping a 1T THROUGHPUT isolate: a bench-identical correctness repair can still be a pure speed regression, and the games only looked like a coherence verdict. Measure NPS at identical nodes before attributing a game result to the mechanism -- BOTH candidates here were bench-identical and BOTH were pure speed regressions. A correctness repair has a price, and that price is evidence: when it exceeds the measured harm, the honest close is an accepted, precisely documented risk, not a repair shipped because repairs feel safer. Record the accepted risk where the next reader will hit it -- the code comment that carried the false claim -- not only in the ledger. | `src/tt.h`; `tests/test_tt.cpp`; `DESIGN.md` s3; `analysis/tt_publication_coherence_v1.md`; 9.3 |
| BAS-C06 | Positive Syzygy tests depended on `D:\\chess\\Syzygy345`; when that private directory was absent they reported passing placeholder assertions without probing WDL, DTZ, rule-50 normalization, root metadata, TB hits, or PV expansion. | **Test-instrument repair accepted; search unchanged.** A repository fixture now carries the canonical 272-byte KQvK WDL and 5,392-byte DTZ tables as base64 (SHA-256 `517667df...2d50` and `71ea9444...1c8c`). Tests materialize them in a unique temporary directory and hard-fail initialization or decoding errors; all nine positive Syzygy assertions ran in release and ASan/UBSan builds. **Coverage caveat added 2026-09-07:** KQvK holds no `WDL == +/-1` entries, so the cursed-win and blessed-loss decode path is covered by nothing in the repository. The section formerly named "rule50 cursed win" tests rule-50 CLAMPING of a clean WDL 2 win at halfmove clock 99, a different mechanism; it was renamed and the gap recorded in `tests/fixtures/syzygy/README.md`. This matters because `endgame_truth.py` scores a move that downgrades a clean win to a cursed win as DISCARDING the win, so a WDL +/-1 decoding defect would pass the whole suite while corrupting the endgame instrument's central judgement. Closing it needs a five-man table such as KNNvKP, far too large to embed; the gap is accepted and recorded rather than implied to be covered. | A positive integration test must own its smallest valid external-data fixture. Absence of a developer-local resource is a failure unless the test is explicitly classified optional. A test's NAME is part of its contract: one that implies coverage it does not have is a quieter version of the placeholder assertion it replaced. | `tests/test_search.cpp`; `tests/fixtures/syzygy/` |
| BAS-C07 | `Board` exposed writable bitboards, occupancies, mailbox, king/check caches, incremental keys and undo history even though those fields are one redundant position representation. Any caller could mutate one view without repairing the others. | **API containment accepted; search unchanged.** Canonical and derived storage is private, hot consumers use inline read-only scalar/array views, and compile-time guards reject renewed public exposure. Repository-wide callers compile, release and ASan/UBSan suites pass 12/12, and the exact 1T bench remains 12,568,898. | Redundant chess state needs one mutation authority. Read-only views preserve hot-path code generation while preventing consumers and tests from manufacturing illegal internal states. No SPRT is warranted for a behavior-neutral access-control change with exact deterministic identity. | `src/board.h`; `tests/test_board.cpp` |
| BAS-C08 | **SEE king legality (15.0.a).** BAS-X29 reported that Basilisk's `see_ge` "has no king rule". Measurement corrected the mechanism: the `KING=20000` sentinel plus the minimax fold already implemented the rule, but it inferred the opponent's reply from the 8.2 **pin-filtered** attacker set. A piece pinned against its own king still controls squares against the enemy king, so a pinned defender was dropped from that set and the king recaptured illegally. All seven king-legality fixtures ported from Rarog's `see-contract-v1`/`see-repair-v1` **passed on the unrepaired kernel**; only the new pinned-defender cases fail first. | **Repaired and closed 2026-09-09.** Both kernels break before a king capture when the **unfiltered** opponent attacker set is non-empty. Over 1,896,743 captures from a seeded 4,000-game random-walk corpus the kernel was diffed against itself: 6,919 `see()` values and 333 `see_ge` threshold verdicts changed, and an independent make/unmake legality oracle scores the repair **6,481-0** and **301-0** with zero regressions. `see`/`see_ge` disagreement fell 4.579% -> 4.236%. CTest 12/12. **Bench 12,568,898 -> 14,978,465 nodes (+19.17%)**, NPS -1.28%. Strength undecided; carried to BAS-E55. | Pin filtering answers "may this piece recapture?"; king-move legality answers "is this square controlled?". They are different questions and a single attacker set cannot serve both -- reusing the recapture filter for legality silently under-defends. A sentinel that encodes a rule implicitly is also a rule nobody can see is reading the wrong input; the donor's explicit two-line form is worth more than its equivalence suggests. Note the cost direction: a SEE correctness repair that raises SEE verdicts **loosens** every consumer that prunes on `see_ge == FALSE`, so "more correct" bought a 19% larger tree, tail-driven (per-position median +3.6%, 15 of 40 positions shrank). | 15.0.a, BAS-X22, BAS-X29 |
| BAS-C09 | **Created pins and recapture promotions (15.0.c).** BAS-X29's remaining two SEE defects, ported from Rarog's `see-contract-v1`/`see-repair-v1`. Measured, both are narrower than their names suggest. **Created pins:** three of the four fixtures already PASS -- `see_pins` runs on the exchange occupancy with the mover already removed, so a pin opened by the mover's own departure is honoured. Only a pin opened mid-exchange by a LATER capturer fails, and only in `see_ge`; `see()` reaches the right answer because its X-ray truncation happens to stop first. **Promotion recaptures:** the kernel adds no promotion gain and leaves a pawn rather than a queen on the square. Each error is worth exactly (queen - pawn) = 800, so they **cancel exactly whenever the promoted piece is immediately recaptured**; the 800 survives only when the promoted piece stands. | **CLOSED 2026-09-10 as a documented approximation, no engine source change; bench unchanged at 14,978,465.** Branch (2) of the leaf is triggered twice over. **Not reachable:** across 339,607 production `see_ge` calls (the 40 bench positions at depth 13, engine instrumented to recompute pins per ply and compare verdicts), created-pin verdict changes = **0** and promotion recaptures = **0**. On a deliberately adversarial random-walk corpus that drifts to sparse, pawn-heavy endgames -- 5,728,160 `see_ge` calls -- created-pin verdict changes are **172 (0.0030%)** and promotion recaptures **1,018 (0.018%)**, of which only the non-cancelling subset diverges. **Over cost:** the current-occupancy recapturer (recompute `see_pins` every ply) measures **+16.8% of the SEE column** in `tests/board_performance.cpp` (7,650,000 -> 6,547,556 captures; an independent micro-benchmark agreed at +17.8%), against the leaf's 10% ceiling. The promotion repair carries a second, separate cost not paid here: it invalidates `see_ge`'s `swap <= 0 -> return true` fast exit, which would have to be gated on last-rank targets -- 8.9% of production calls. Nine fixtures added to `tests/test_board.cpp`; the test oracle is now promotion-aware, and the three diverging cases pin BOTH the oracle truth and the kernel's approximate answer so the divergence cannot drift or be silently 'fixed'. | A defect's name is not its frequency. 'Created pins' sounded like one mechanism and is two: the mover-departure case was already correct and only the mid-exchange case fails, which is a rarer geometry by an order of magnitude. Check which fixtures already pass BEFORE pricing a repair. Second: two errors of equal magnitude and opposite sign are not two defects -- the promotion-recapture pair cancels exactly in the common continuation, so the reachable error is far smaller than either term. Third: correctness that no caller can reach is not worth 16.8% of a hot column, and the honest close is a pinned, documented approximation rather than a repair shipped because repairs feel safer (BAS-C05 doctrine). **Retry trigger:** reopen if production `see_ge` verdict changes exceed 0.1% of calls on the bench corpus after any selectivity or endgame-search change, or if an INCREMENTAL created-pin update (rather than a full per-ply `see_pins`) measures at or under 10% of the SEE column. | 15.0.c, BAS-X29, BAS-X22, BAS-C08 |
